...
def ensemble_test(pp=5, qbit=8):

    net = EnsembleNet(res_stop=pp, ncls=100, qbit=qbit).to(device)
    ...
    for num_of_ens in range(num_users):
        ...
        checkpoint = torch.load(f'./checkpoint/0.0001_{pp}_100_16_{num_of_ens}_{args.qbit}_AdaptE_ckpt.pth')
        # checkpoint = torch.load('./checkpoint/ckpt_74.49.pth')
        checkpoint_2 = torch.load(f'./checkpoint/0.0001_{pp}_100_16_-1_{args.qbit}_AdaptE_ckpt.pth')
        ...
        with torch.no_grad():
            for b, (X_test, y_test) in enumerate(testloader):
                ...
                y_hat_tensor[num_of_ens, b, :, :] = net(X_test)
...
if __name__ == "__main__":
    ...
    ensemble_test(pp=args.pp, qbit=args.qbit)
